...
import {
  getOrganizationReferralReceivedApi,
  getOrganizationReferralSentApi,
  postOrganizationReferralClaimApi,
} from "@/apis/ApiCalls";
import { ClaimReferralModal } from "@/components/staffComponents/ClaimReferralModal";
...
export type SenderInboxBody = {
  page: number;
  limit: number;
  search: string;
  type: SenderInboxType;
  day: number;
};
export type ReceiverInboxBody = {
  page: number;
  limit: number;
  search: string;
  type: ReceiverInboxType;
  day: number;
};
...
export default function StaffInboxPage() {
  ...
  const [senderBody, setSenderBody] = useState<SenderInboxBody>({
    page: 1,
    limit: 10,
    search: "",
    type: "all",
  ...
  const [receiverBody, setReceiverBody] = useState<ReceiverInboxBody>({
    page: 1,
    limit: 10,
    search: "",
    type: "all",
  ...
  const { isPending: isClaimPending, mutateAsync: claimReferral } = useMutation(
    {
      mutationFn: catchAsync(async (code: string) => {
        const res = await postOrganizationReferralClaimApi(code);
  ...
  const { data: sentResponse, isLoading: isLoadingSent } = useQuery({
    queryKey: [
      ...defaultQueryKeys.referralSentList,
      senderBody.page,
      senderBody.search,
      senderBody.type,
      senderBody.day,
    ...
    queryFn: async (): Promise<ReferralListResponse<SentReferralApi>> => {
      ...
      const meta = raw?.meta ?? {
        ...defaultMeta,
        total: data.length,
        totalPages: Math.ceil((data.length || 1) / (senderBody.limit || 10)),
      };
      return { data, meta };
    },
  });
  ...
  const { data: receivedResponse, isLoading: isLoadingReceived } = useQuery({
    queryKey: [
      ...defaultQueryKeys.referralReceivedList,
      receiverBody.page,
      receiverBody.limit,
      receiverBody.search,
      receiverBody.type,
    ...
    queryFn: async (): Promise<ReferralListResponse<ReceivedReferralApi>> => {
      ...
      const meta = raw?.meta ?? {
        ...defaultMeta,
        total: data.length,
        totalPages: Math.ceil((data.length || 1) / (receiverBody.limit || 10)),
      };
      return { data, meta };
    },
  });
  ...
  const baseReceivedReferrals = useMemo(
    ...
    <div className="max-w-[1280px] mx-auto p-[18px]">
      ...
      <div className="flex flex-wrap gap-3 items-center justify-between p-3.5 px-4 border border-slate-200 bg-white/80 backdrop-blur-md rounded-2xl shadow-[0_10px_30px_rgba(2,6,23,.07)] sticky top-2.5 z-10">
        <div className="flex items-center gap-2.5 min-w-[260px]">
          <div>
            <h1 className="m-0 text-[15px] font-semibold leading-tight tracking-wide">
              Referral Coordination Network
            </h1>
            <p className="m-0 text-xs text-rcn-muted font-bold">
              Website Inbox • Sender + Receiver
            </p>
          </div>
        </div>

        <div
          className="flex bg-white border border-slate-200 rounded-full p-1 gap-1 shadow-[0_8px_20px_rgba(2,6,23,.06)]"
          role="tablist"
        ...
        <div className="flex items-center gap-2.5 flex-wrap justify-end">
          ...
          <Link
            ...
            className="border border-rcn-brand/25 bg-rcn-brand/10 text-rcn-accent-dark px-3 py-2.5 rounded-xl font-extrabold text-xs shadow-[0_10px_22px_rgba(2,6,23,.06)] whitespace-nowrap no-underline"
          >
            + New Referral
          </Link>
        </div>
