...
import { AdminPermission, AdminProfileData } from "@/app/master-admin/types/profile";
...
const loginSchema = yup.object({
  ...
  email: yup.string().trim().required("Please enter email.").email("Please enter a valid email."),
...
const Login: React.FC = () => {
  ...
  const [pendingOtp, setPendingOtp] = React.useState<PendingOtpData | null>(null);
  ...
  const { isPending, mutate: mutateLogin } = useMutation({
    mutationFn: catchAsync(
      async (variables: LoginFormValues) => {
        ...
        if (checkResponse({ res, showSuccess: true })) {
          ...
          const redirectTo =
            type === "user" ? "/staff-portal" : type === "org" ? "/org-portal" : "/master-admin/dashboard";
    ...
    mutationFn: catchAsync(
      async ({ pending, otp }: { pending: PendingOtpData; otp: string }) => {
        ...
        if (checkResponse({ res, showSuccess: true })) {
          ...
          const loginUser: AuthProfileData | StaffProfileData | AdminProfileData | null =
            loginType === "user" 
              ? data?.user ?? null
              : loginType === "org" 
                ? data?.organization ?? null
                : data?.admin 
                  ? { ...data.admin, permissions: data.permissions ?? [] }
                  : null;
      ...
      <div className="max-w-[980px] w-full">
        <div className="grid grid-cols-1 md:grid-cols-[1.1fr_0.9fr] gap-4">
          <div className="rounded-rcn-lg p-5 login-hero-gradient text-rcn-dark-text shadow-rcn min-h-[280px]">
            <div className="flex gap-3 items-center">
              ...
              <div>
                ...
                <p className="text-rcn-dark-text/85 text-sm m-0">Admin Panel (Demo — No API)</p>
              </div>
            </div>
          ...
          <div className="bg-white border border-rcn-border rounded-rcn-lg shadow-rcn p-4">
            ...
            <Modal isOpen={!!pendingOtp} onClose={closeOtpModal} maxWidth="400px" locked={false}>
              <div className="p-2 w-full">
                ...
                <form onSubmit={onSubmitOtp} className="mt-5">
                  ...
                  <div className="flex gap-2 mt-5 justify-end">
                    <Button variant="secondary" size="sm" type="button" onClick={closeOtpModal}>
                      Cancel
                    </Button>
                    <Button
                      variant="primary"
                      size="sm"
                      type="submit"
                      disabled={otpDigits.join("").length !== OTP_LENGTH || isVerifyPending}
                    >
            ...
            <div className="mb-4">
              <p className="text-xs text-rcn-muted m-0 mb-3">Don&apos;t have an organization account?</p>
